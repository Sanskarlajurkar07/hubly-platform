...
const ChatCenter = () => {
  ...
  const [missedChatTimer, setMissedChatTimer] = useState(10); // Default 10 mins
  const { user } = useAuth();
  const messagesEndRef = useRef(null);

  // Fetch settings for missed chat timer
  useEffect(() => {
    const fetchSettings = async () => {
      try {
        const res = await api.get('/settings');
        if (res.data.missedChatTimer) {
          setMissedChatTimer(res.data.missedChatTimer);
        }
      } catch (error) {
    ...
    <div className="chat-center-container">
      ...
      <div className="chat-sidebar">
        ...
        <div className="chat-list">
          {tickets.map(ticket => {
            const isMissed = ticket.status !== 'Resolved' &&
              ticket.lastMessageAt &&
              (new Date().getTime() - new Date(ticket.lastMessageAt).getTime() > missedChatTimer * 60 * 1000);
      ...
      <div className="chat-main">
        {selectedTicket ? (
          (() => {
            const isRestricted = user.role === 'admin' && selectedTicket.assignedTo && selectedTicket.assignedTo._id !== user._id;
              ...
              <>
                ...
                <div className="chat-messages-area">
                  {messages.map((msg, idx) => {
                    const currentDate = new Date(msg.createdAt || msg.timestamp).toLocaleDateString('en-US', {
                      month: 'long',
                      day: 'numeric',
                      year: 'numeric',
                    });
                    const prevDate =
                      idx > 0
                        ? new Date(messages[idx - 1].createdAt || messages[idx - 1].timestamp).toLocaleDateString(
                          'en-US',
                          {
                            month: 'long',
                            day: 'numeric',
                    ...
                    return (
                      <React.Fragment key={idx}>
                        ...
                        <div className={`message-row ${msg.sender === 'agent' ? 'agent' : 'customer'}`}>
                          {msg.sender === 'customer' && (
                ...
                {selectedTicket.status === 'Resolved' ? (
                  <div style={{ padding: '20px', textAlign: 'center', borderTop: '1px solid #e2e8f0', backgroundColor: '#f8fafc' }}>
                  ...
                  <div className="chat-input-wrapper">
                    {selectedTicket && selectedTicket.lastMessageAt && (new Date().getTime() - new Date(selectedTicket.lastMessageAt).getTime() > missedChatTimer * 60 * 1000) && (
                      <div style={{ textAlign: 'center', color: '#ef4444', fontSize: '12px', marginBottom: '8px', fontWeight: '600' }}>
